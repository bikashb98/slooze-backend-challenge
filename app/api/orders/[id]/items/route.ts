...
export async function POST(
  ...
  try {
   const user = await authMiddleware(request);
    ...
    const item = await addItem(orderId, menuItemId, quantity, user.role, user.country);
    return NextResponse.json(item, { status: 201 });
  } catch (error) {
    if (error instanceof Error) {
        if (error.message === "User Forbidden") {
            return NextResponse.json(
                { error: "Only Admins can add items to order from restaurants other than your country", message: error.message },
                { status: 403 },
            );
        }
