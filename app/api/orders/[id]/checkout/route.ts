...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
    try {
        ...
        return NextResponse.json({"message": "Order checked out successfully", order}, { status: 200 });
    } catch (error) {
        if (error instanceof Error) {
            if (error.message === "Forbidden") {
                return NextResponse.json({ error: "Only admin and manager can checkout orders", message: error.message }, { status: 403 });
            }
